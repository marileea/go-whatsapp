...
func APIKeyAuth(cfg APIKeyConfig) fiber.Handler {
    ...
    return func(c *fiber.Ctx) error {
        ...
        if !allowResult.Allowed {
            ...
            cfg.logRequest(apiKey.ID, tenant.ID, assignment.ID, c, fiber.StatusTooManyRequests, 0, fmt.Errorf("rate limit exceeded"))
        ...
        tenantCtx := &domainTenant.TenantContext{
            TenantID:           tenant.ID,
            APIKeyID:           apiKey.ID,
            ServerAssignmentID: assignment.ID,
        ...
        if err := c.Next(); err != nil {
            cfg.logRequest(apiKey.ID, tenant.ID, assignment.ID, c, c.Response().StatusCode(), time.Since(started), err)
            return err
        }
        ...
        cfg.logRequest(apiKey.ID, tenant.ID, assignment.ID, c, statusCode, time.Since(started), nil)
...
func parseInt(value interface{}) int {
    ...
    case string:
        ...
        if parsed, err := fmt.Sscanf(v, "%d", new(int)); err == nil {
            return parsed
        }
    }
    return 0
